...
// To prevent outsiders from using the command, try configuring RequireUserPermission.
...
public class AdminModule : InteractionModuleBase<SocketInteractionContext>
{
        ...
        [Summary("amount", $"The number of messages to clean up.")] int amount)
    {
        ...
        var messages = await Context.Channel.GetMessagesAsync(amount + 1).FlattenAsync(); // Add bot's response to total amount of messages.
        // Due to Discord's limits, it is only possible to delete messages which are less than two weeks old.
        var youngMessages = messages.Where(x => x.Timestamp > DateTime.Now.AddDays(-14)).Skip(1).ToList(); // Skip bot's response.
